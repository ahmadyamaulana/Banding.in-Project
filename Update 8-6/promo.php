    ...
    <script>
        ...
        tombolBanding.addEventListener("click", function () {
            ...
            semuaCardItem.forEach(function (cardItem, index) {
                ...
                // 1. Definisikan card dan ambil nilai input dari masing-masing card secara dinamis
                const card = cardItem.querySelector('.card');
                const nama = card.querySelector('input[placeholder="Nama Produk"]').value.trim();
                const diskonText = card.querySelector('input[placeholder="Masukan Diskon"]').value.trim();
                const hargaText = card.querySelector('input[placeholder="Harga"]').value.trim();

                const imgArea = card.querySelector('.image-upload-area');
                const imgTag = imgArea.querySelector('img');
                let gambarSrc = "";

                // Cek apakah src mengandung format Base64 (data:image)
                if (imgTag && imgTag.src && imgTag.src.startsWith('data:image')) {
                    gambarSrc = imgTag.src;
                }

                // 2. Konversi teks input menjadi angka numerik
                const diskon = parseFloat(diskonText) || 0;
                const harga = parseFloat(hargaText) || 0;

                // 3. Hanya proses kartu yang sudah diisi nama produk dan harganya
                if (nama !== "" && harga > 0) {
                    adaData = true;
                    const hargaAkhir = harga - (harga * diskon / 100);
            ...
            if (adaData) {
                ...
                alert("Isi nama produk dan harga terlebih dahulu!");
            }
        });
